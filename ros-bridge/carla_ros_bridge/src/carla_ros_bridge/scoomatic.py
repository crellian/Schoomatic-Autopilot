...
import numpy
from carla import ScoomaticControl
...
class Scoomatic(Vehicle):
    ...
    def __init__(self, uid, name, parent, node, carla_actor, vehicle_control_applied_callback):
        ...
        """
        super(Scoomatic, self).__init__(uid=uid,
                                         name=name,
                                         parent=parent,
                                         node=node,
    ...
    def send_vehicle_msgs(self, frame, timestamp):
        ...
        vehicle_status.control.throttle = self.carla_actor.get_control().left_velocity
        vehicle_status.control.steer = self.carla_actor.get_control().right_velocity
    ...
    def update(self, frame, timestamp):
        ...
        super(Scoomatic, self).update(frame, timestamp)
    ...
    def control_command_updated(self, ros_vehicle_control, manual_override):
        ...
        if manual_override == self.vehicle_control_override:
            scoomatic_control = ScoomaticControl()
            scoomatic_control.left_velocity = (ros_vehicle_control.throttle - ros_vehicle_control.steer * 4) * 1000
            scoomatic_control.right_velocity = (ros_vehicle_control.throttle + ros_vehicle_control.steer * 4) * 1000

            self.carla_actor.apply_control(scoomatic_control)
    ...
    def get_vehicle_speed_squared(carla_vehicle):
        ...
        :rtype: float64
        """
        return Scoomatic.get_vector_length_squared(carla_vehicle.get_velocity())
    ...
    def get_vehicle_speed_abs(carla_vehicle):
        ...
        :rtype: float64
        """
        speed = math.sqrt(Scoomatic.get_vehicle_speed_squared(carla_vehicle))
        return speed
